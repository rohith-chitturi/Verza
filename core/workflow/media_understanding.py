    ...
    def _execute(self, context: AIContext, trace_id: str, **kwargs) -> AIContext:
        ...
        return context.with_world(
            context.world.with_visual(
                context.world.visual.model_copy(update={"scenes": scenes})
            )
        )
...
class MockSemanticGraphCapability(BaseCapability):
    @property
    def name(self) -> str:
        return "SemanticGraph(Mock)"

    def _execute(self, context: AIContext, trace_id: str, **kwargs) -> AIContext:
        kg = {"character-001": {"talking_to": "character-002"}}
        return context.with_world(
            context.world.with_semantic(
                context.world.semantic.model_copy(update={"knowledge_graph": kg})
            )
        )
...
class MockWorldSynthesisCapability(BaseCapability):
    @property
    def name(self) -> str:
        return "WorldStateSynthesis(Mock)"
...
class MediaUnderstandingEngine:
    ...
    def execute(self, initial_context: AIContext) -> AIContext:
        ...
        logger.info(
            "m2_workflow_completed",
            trace_id=trace_id,
            final_world=current_context.world.model_dump(),
        )
