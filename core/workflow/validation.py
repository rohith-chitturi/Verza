...
class WorkflowValidator:
    ...
    def __init__(
        self, registered_capabilities: set[str], registered_providers: set[str]
    ):
    ...
    def _check_cycles(self, workflow: Workflow) -> None:
        ...
        def visit(node: str) -> None:
            if node in path:
                raise WorkflowValidationError(
                    f"Circular dependency detected involving stage: '{node}'"
                )
            if node in visited:
                return
    ...
    def _check_capabilities_and_providers(self, workflow: Workflow) -> None:
        for stage in workflow.stages:
            if stage.capability not in self._registered_capabilities:
                raise WorkflowValidationError(
                    f"Stage '{stage.id}' requires unknown capability: '{stage.capability}'"
                )

            policy = stage.provider_policy
            if policy.primary not in self._registered_providers:
                raise WorkflowValidationError(
                    f"Stage '{stage.id}' requires unknown primary provider: '{policy.primary}'"
                )
            for fallback in policy.fallback:
                if fallback not in self._registered_providers:
                    raise WorkflowValidationError(
                        f"Stage '{stage.id}' requires unknown fallback provider: '{fallback}'"
                    )
