    ...
    def merge(self, state: WorldState, delta: WorldStateDelta) -> WorldState:
        ...
        for op in delta.operations:
            ...
            if len(domain_parts) < 2 or len(domain_parts) > 3:
                logger.warning(
                    f"Invalid domain format {op.domain}, skipping operation {op.change_id}"
                )
                continue
            ...
            if section not in state_dict or collection not in state_dict[section]:
                logger.warning(
                    f"Unknown domain path {op.domain}, skipping operation {op.change_id}"
                )
                continue
            ...
            if len(domain_parts) == 3:
                ...
                if sub_collection not in target:
                    logger.warning(
                        f"Unknown sub-domain {op.domain}, skipping operation {op.change_id}"
                    )
                    continue
            ...
            if op.operation == Operation.ADD or op.operation == Operation.LINK:
                ...
                if isinstance(target, list):
                    ...
                    items_to_remove = [
                        item for item in target if item.get("id") == op.entity_id
                    ]
