...
class ConsistencyChecker:
    ...
    def check(
        self, delta: WorldStateDelta, current_state: WorldState
    ) -> ConsistencyResult:
            ...
            if op.domain == "semantic.knowledge_graph.edges":
                ...
                if op.operation == Operation.ADD or op.operation == Operation.LINK:
                    ...
                    for existing_edge in kg.edges:
                        if (
                            existing_edge.source == source
                            and existing_edge.target == target
                            and existing_edge.relation == relation
                        ):
                            # If temporal bounds overlap, it's a duplicate or superseding
                            # Simplifying for now:
                            logger.info(
                                f"Duplicate relation detected: {source} {relation} {target}"
                            )
                            return ConsistencyResult(
                                ConsistencyState.DUPLICATE, "Relation already exists"
                            )
                        ...
                        if (
                            existing_edge.source == source
                            and existing_edge.target == target
                            and existing_edge.relation == "FRIEND_OF"
                            and relation == "HOSTILE_TOWARDS"
                        ):
                            # This might be superseding if time has passed, or conflicting
                            logger.info(
                                f"Potential conflict/superseding: {source} {relation} {target} vs {existing_edge.relation}"
                            )
                            return ConsistencyResult(
                                ConsistencyState.SUPERSEDING,
