...
class DeltaValidator:
    ...
    def _validate_schema(self, delta: WorldStateDelta) -> None:
        ...
        for op in delta.operations:
            ...
            if op.operation in [Operation.UPDATE, Operation.REMOVE] and not op.entity_id:
                raise ValidationError(f"Change ID {op.change_id} ({op.operation}) requires an entity_id.")
    ...
    def _validate_semantic(self, delta: WorldStateDelta) -> None:
        ...
        for op in delta.operations:
            ...
            if op.confidence.confidence < 0.0 or op.confidence.confidence > 1.0:
                raise ValidationError(f"Confidence score {op.confidence.confidence} must be between 0.0 and 1.0.")
