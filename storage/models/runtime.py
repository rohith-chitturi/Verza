...
class WorkflowRunModel(Base):
    ...
    parent_run_id: Mapped[str | None] = mapped_column(ForeignKey("workflow_runs.id"), nullable=True) # For replay lineage
    ...
    # Relationships
    stage_runs: Mapped[list["StageRunModel"]] = relationship(back_populates="workflow_run")
...
class StageRunModel(Base):
    ...
    attempts: Mapped[list["TaskAttemptModel"]] = relationship(back_populates="stage_run")
...
class ExecutionEventModel(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    workflow_run_id: Mapped[str] = mapped_column(ForeignKey("workflow_runs.id"))
    event_type: Mapped[str] = mapped_column(String, nullable=False) # e.g. STAGE_STARTED, RUN_PAUSED
