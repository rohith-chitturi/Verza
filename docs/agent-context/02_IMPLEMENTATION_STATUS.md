...
| **M2 Media Understanding** | YES | YES | YES | YES | YES | NO | `FFmpegMetadataProvider` tests pass with real `ffprobe` execution on physical media. `WhisperRecognizer` tests pass with real `faster-whisper` execution on physical speech fixtures. Verified in `test_m4_real_execution.py` and `test_m2_whisper.py`. |
