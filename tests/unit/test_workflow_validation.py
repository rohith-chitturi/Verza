...
def test_unknown_capability(validator):
    ...
    with pytest.raises(
        WorkflowValidationError, match="requires unknown capability: 'unknown_cap'"
    ):
        validator.validate(workflow)
    ...
    workflow = Workflow(
        ...
        stages=[
            Stage(
                id="s1",
                capability="media_understanding",
                provider_policy=ProviderPolicy(primary="unknown_provider"),
            )
        ],
    )
    with pytest.raises(
        WorkflowValidationError,
        match="requires unknown primary provider: 'unknown_provider'",
    ):
        validator.validate(workflow)
